...
export class BookServiceController {
  ...
  public CreateService: RequestHandler = async (req, res, next) => {
    const token = req.headers.authorization;
      ...
      return this.bookService
        ...
        .then((user) => {
          ...
          return this.bookService
            ...
            .then((service) => {
              return this.bookService
                .getHelperByZC(req.body.ZipCode)
                .then((sp) => {
                  if(sp) {
                    for(let email in sp) {
                      const serviceRequest = this.bookService.serviceRequest(sp[email].Email!);
